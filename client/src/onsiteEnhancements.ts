...
function installStyles() {
  ...
  style.textContent = `
    ...
    .onsite-pete-strip { margin: 0 auto 22px; max-width: 1100px; border: 1px solid rgba(249,115,22,.28); border-radius: 18px; padding: 18px 20px; background: linear-gradient(135deg,rgba(249,115,22,.14),rgba(15,23,42,.72)); display:flex; align-items:center; gap:16px; box-shadow:0 18px 50px rgba(0,0,0,.22); }
    ...
    .onsite-loader-card { width:min(520px,100%); border:1px solid rgba(249,115,22,.35); border-radius:24px; padding:30px; background:linear-gradient(160deg,rgba(20,32,55,.98),rgba(9,16,31,.98)); text-align:center; box-shadow:0 30px 90px rgba(0,0,0,.55); }
    ...
    .onsite-loader-tagline { margin-top:12px !important; color:#fb923c !important; font-weight:700; }
...
export function installOnsiteEnhancements() {
  installStyles();
  const refresh = () => { replaceBranding(); addPeteStrip(); };
  refresh();
